...
def Colaborador(event=None):

    Conexao = pymysql.connect(host="localhost", user="root", passwd="P@ssw0rd", db="BD_SYSTEM")
    Cursor_Id_Colab = Conexao.cursor()
    Cursor_Id_Colab.execute("SELECT NAME_USER FROM USERS WHERE ID = '%s';" % Var_Id_Colaborador.get())
    for id in Cursor_Id_Colab.fetchall():
        Img_Colaborado.config(text=str(id[0]), fg=Branco)
    Ent_User.config(state=DISABLED, disabledforeground=Preto)
    EntCod_Prod.focus()
...
Ent_User.bind("<Return>", Colaborador)
Ent_User.bind("<Tab>", Colaborador)
Img_Colaborado = Label(FrInf_Cli, bg=Cinza_Novo, fg=Cinza_Novo, font=Fonte11B)
Img_Colaborado.place(x=230, y=150)
